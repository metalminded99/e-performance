...
CREATE TABLE `tbl_emp_process` (
  `emp_proc_id` int(11) NOT NULL AUTO_INCREMENT,
  `user_id` int(11) DEFAULT NULL,
  `process_id` int(11) DEFAULT NULL,
  `date_accomplished` datetime DEFAULT NULL,
  `status` enum('Completed','Pending','On-going','Invalid') DEFAULT 'Pending',
  `date_assigned` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`emp_proc_id`),
  KEY `user_id` (`user_id`),
...
/*Table structure for table `tbl_emp_process_comments` */

DROP TABLE IF EXISTS `tbl_emp_process_comments`;

CREATE TABLE `tbl_emp_process_comments` (
  `emp_proc_id` int(11) DEFAULT NULL,
  `comment` longtext,
